...
def random_extend_route(model: Model, route_id):
    """
    Extends a single route until the max is reached or all possible stations have been visited twice.
...
def random_reorder_route(model: Model, route_id: int):
    """
    Reorders existing route using only the stations in the model. Picks a new starting station
    from the ends of the route.
    """
